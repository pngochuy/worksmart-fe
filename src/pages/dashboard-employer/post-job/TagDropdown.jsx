...
const TagDropdown = ({ setSearchParams, initialTags = [] }) => {
  ...
  // Lấy danh sách tất cả các tags từ API
  const getTags = async () => {
    ...
    // Nếu có initialTags, set selected tags sau khi có options
    if (initialTags && initialTags.length > 0 && options.length > 0) {
      // Tìm các option tương ứng với initialTags
  ...
  // Xử lý khi initialTags thay đổi và tagOptions đã được load
  useEffect(() => {
    if (initialTags && initialTags.length > 0 && tagOptions.length > 0) {
      const initialSelectedOptions = tagOptions.filter(option => 
        initialTags.includes(option.value)
      );
      setSelectedTags(initialSelectedOptions);
    }
  ...
  const handleChange = (selected) => {
    ...
    setSearchParams((prev) => ({
      ...prev,
      jobTagID: selected ? selected.map((tag) => tag.value) : [],
    }));
  };
