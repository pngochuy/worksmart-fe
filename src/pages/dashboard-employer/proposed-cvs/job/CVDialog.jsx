...
const CVDialog = ({ open, onOpenChange, candidate, cv }) => {
  const [showPdfViewer, setShowPdfViewer] = useState(false);

  if (!candidate || !cv) return null;

  // Kiểm tra xem CV có phải là loại 2 (có URL từ Cloudinary) hay không
  const hasCVFile = cv.filePath || cv.cloudinaryUrl;
  const cvFileUrl = cv.filePath || cv.cloudinaryUrl || "";

  // Kiểm tra xem có phải là URL Cloudinary không và có phải là PDF
  const isCloudinaryUrl = cvFileUrl && cvFileUrl.includes("cloudinary.com");
  const isPDF = cvFileUrl && cvFileUrl.toLowerCase().endsWith(".pdf");
  ...
  const downloadCV = () => {
    ...
    try {
      // Xử lý đặc biệt cho file PDF từ Cloudinary
      if (isCloudinaryUrl && isPDF) {
        // Tạo tên file từ URL hoặc dùng tên mặc định
        const filename = cvFileUrl.split("/").pop() || "cv.pdf";

        // Sử dụng fetch để lấy PDF với headers phù hợp
        fetch(cvFileUrl)
          .then((response) => response.blob())
          .then((blob) => {
            // Tạo blob URL và kích hoạt tải xuống
        ...
        // Đối với các file không phải từ Cloudinary, mở trong tab mới
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto border border-blue-100 shadow-lg rounded-xl p-6">
        <>
          ...
          {/* Footer Actions */}
          <div className="flex justify-between mt-6 pt-4 border-t border-blue-100">
            {hasCVFile ? (
              <Button
                variant="outline"
              ...
              <Button
                variant="outline"
                className="bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100 transition-all"
                disabled
              >
                <FileWarning className="mr-2 h-4 w-4" /> No CV File
              </Button>
            )}
            ...
            <div className="space-x-2">
              <Button
                variant="outline"
                className="border-blue-200 text-blue-700 hover:border-blue-300 hover:bg-blue-50 transition-all"
              >
                Send Message
              </Button>
