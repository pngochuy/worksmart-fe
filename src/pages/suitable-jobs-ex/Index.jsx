...
export const Index = () => {
    ...
    const checkForUpdates = async (userId) => {
        try {
            ...
            const lastCVCheck = parseInt(localStorage.getItem(`cv_last_checked_${userId}`) || '0');
            const lastJobCheck = parseInt(localStorage.getItem(`jobs_last_checked`) || '0');
            ...
            if (latestCVData && latestCVData.length > 0) {
                latestCVTimestamp = Math.max(...latestCVData.map(cv => {
                    const updateTime = cv.updatedAt ? new Date(cv.updatedAt).getTime() : 0;
                    const createTime = cv.createdAt ? new Date(cv.createdAt).getTime() : 0;
                    return Math.max(updateTime, createTime);
                }));
            }

            // Lấy thời gian cập nhật công việc mới nhất
            const latestJobTimestamp = parseInt(localStorage.getItem('latest_job_update') || '0');

            console.log("CV check:", { lastCVCheck, latestCVTimestamp, hasUpdates: latestCVTimestamp > lastCVCheck });
            console.log("Job check:", { lastJobCheck, latestJobTimestamp, hasUpdates: latestJobTimestamp > lastJobCheck });

            // Kiểm tra cập nhật
            return {
                hasUpdates: latestCVTimestamp > lastCVCheck || latestJobTimestamp > lastJobCheck,
                latestCVTimestamp,
                latestJobTimestamp
            };
        } catch (err) {
    ...
    const fetchData = async () => {
        try {
            ...
            if (cachedData) {
                ...
                if (cachedData.recommendations && Array.isArray(cachedData.recommendations)) {
                    cachedData.recommendations = cachedData.recommendations.filter(job => {
                        return job &&
                            typeof job === 'object' &&
                            job.jobId &&
                            job.jobTitle &&
    ...
    const refreshRecommendations = async () => {
        try {
            ...
            localStorage.setItem(`cv_last_checked_${userId}`, updateResult.latestCVTimestamp.toString());
    ...
    const renderSkeletonLoader = () => {
        return (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {[1, 2, 3, 4].map((_, index) => (
                    <div key={index} className="p-5 border rounded-lg bg-white animate-pulse">
    ...
    const renderRecommendationList = () => {
        ...
        const validRecommendations = recommendations.recommendations.filter(job => {
            return job &&
                typeof job === 'object' &&
                job.jobId &&
                job.jobTitle &&
        ...
        return (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {validRecommendations.map((job, index) => (
                    <div key={index} className="p-5 border rounded-lg bg-white shadow-sm hover:shadow-md transition-all duration-300">
                        <div className="flex justify-between items-start">
                            <div className="w-3/4">
                                <h3 className="font-bold text-lg text-gray-800 line-clamp-1">{job.jobTitle}</h3>
                            ...
                            <span className={`px-4 py-2 rounded-full font-medium text-sm ${job.matchPercentage >= 80
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
                ...
                <div className="bg-white rounded-xl shadow-md overflow-hidden p-6 mb-8">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <h2 className="text-2xl font-bold text-gray-800">Công việc đề xuất</h2>
                        <button
                            onClick={refreshRecommendations}
                            disabled={loading || refreshing}
                            className={`flex items-center gap-2 px-4 py-2 rounded-lg ${loading || refreshing ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'
                    ...
                    {hasUpdates && (
                        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span className="ml-1">CV hoặc danh sách công việc đã được cập nhật. Nhấn "Cập nhật ngay" để xem đề xuất mới nhất.</span>
                            </div>
                        </div>
                    )}
                        ...
                        <div>
                            <div className="flex items-center justify-center mb-6">
                                <div className="flex items-center gap-3 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg">
                                    ...
                                    <span>Đang phân tích CV và tìm kiếm công việc phù hợp...</span>
                                </div>
                            </div>
                ...
                {userCVs.length > 0 && (
                    <div className="bg-white rounded-xl shadow-md overflow-hidden p-6">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-gray-800">CV của bạn ({userCVs.length})</h2>
                            <Link to="/candidate/my-cv" className="text-blue-600 hover:text-blue-800 flex items-center gap-1 text-sm font-medium">
                                Quản lý CV
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                </svg>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {userCVs.map((cv) => (
                                <div key={cv.cvid} className="p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                            <h3 className="font-medium text-gray-800 line-clamp-1">{cv.title || cv.fileName || `CV #${cv.cvid}`}</h3>
                                            {cv.updatedAt && (
                                    ...
                                    <div className="mt-3 flex justify-end">
                                        <Link to="/candidate/my-cv" className="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1">
                                            Xem CV
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
